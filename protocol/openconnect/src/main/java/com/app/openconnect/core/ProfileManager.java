...
public class ProfileManager {
        ...
        if (prefsdir.exists() && prefsdir.isDirectory()) {
            for (String s : prefsdir.list()) {
                if (s.startsWith(PROFILE_PFX)) {
                    SharedPreferences p = context.getSharedPreferences(s.replaceFirst(".xml", ""), Activity.MODE_PRIVATE);
    ...
    private static String makeProfName(String s, int index) {
        ...
        if ((s.matches("[0-9.]+") && s.matches(".*\\..*")) || (s.matches("[0-9a-fA-F:]+") && s.matches(".*:.*"))) {
            return s + suffix;
        }
    ...
    public synchronized static boolean delete(String uuid) {
        ...
        File f = new File(mContext.getApplicationInfo().dataDir + File.separator + "shared_prefs" + File.separator + PROFILE_PFX + uuid + ".xml");
    ...
    public synchronized static void setConnectedVpnProfile(VpnProfile connectedProfile) {
        ...
        mAppPrefs.edit().putString(ON_BOOT_PROFILE, connectedProfile.getUUIDString()).commit();
    }
