...
import com.app.openconnect.core.UserDialog;
...
public class AuthFormHandler extends UserDialog implements DialogInterface.OnClickListener, DialogInterface.OnDismissListener {
    ...
    public AuthFormHandler(SharedPreferences prefs, LibOpenConnect.AuthForm form, boolean authgroupSet, String lastFormDigest) {
        super(prefs);
    ...
    private LinearLayout.LayoutParams fillWidth = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
    ...
    private LinearLayout newTextBlank(LibOpenConnect.FormOpt opt, String defval) {
        ...
        int baseType = (opt.flags & LibOpenConnect.OC_FORM_OPT_NUMERIC) != 0 ? InputType.TYPE_CLASS_NUMBER : InputType.TYPE_CLASS_TEXT;
    ...
    public void onStart(Context context) {
        ...
        if ((batchMode == BATCH_MODE_EMPTY_ONLY && mAllFilled) || batchMode == BATCH_MODE_ENABLED || !hasUserOptions) {
            saveAndStore();
            finish(LibOpenConnect.OC_FORM_RESULT_OK);
            return;
        }

        mAlert = new AlertDialog.Builder(mContext).setView(v).setTitle(mContext.getString(R.string.login_title, getStringPref("profile_name"))).setPositiveButton(R.string.ok, h).setNegativeButton(R.string.cancel, h).create();
