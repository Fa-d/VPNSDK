...
import org.strongswan.android.Controller;
import org.strongswan.android.VpnProfileControlActivity;
...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener
{
	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		if (intent != null)
		{
			...
			{	/* triggered when Always-on VPN is activated */
				...
				profile = Controller.INSTANCE.getMProfile(); /*mDataSource.getVpnProfile(uuid);*/
			}
			else if (!DISCONNECT_ACTION.equals(intent.getAction()))
			{
				...
				if (bundle != null)
				{
					profile = Controller.INSTANCE.getMProfile(); /*mDataSource.getVpnProfile(bundle.getString(VpnProfileDataSource.KEY_UUID));*/
					if (profile != null)
					{
	 ...
	 */
	private Notification buildNotification(boolean publicVersion) throws ClassNotFoundException {
		...
		//Intent intent = new Intent(getApplicationContext(), Class.forName(Controller.INSTANCE.getPendingIntent()));
		PendingIntent pending = PendingIntent.getActivity(getApplicationContext(), 0, Controller.INSTANCE.getIntent(),
														  flags);
	...
	@Override
	public void stateChanged() throws ClassNotFoundException {
	...
	public class BuilderAdapter
	{
		...
		private Builder createBuilder(String name)
		{
			...
			//Intent intent = new Intent(context, VpnProfileControlActivity.class);
			int flags = PendingIntent.FLAG_UPDATE_CURRENT;
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
			{
				flags |= PendingIntent.FLAG_IMMUTABLE;
			}
			PendingIntent pending = PendingIntent.getActivity(context, 0, Controller.INSTANCE.getIntent(), flags);
