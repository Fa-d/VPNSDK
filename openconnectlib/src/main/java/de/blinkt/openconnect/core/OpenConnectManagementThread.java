...
package de.blinkt.openconnect.core;
...
public class OpenConnectManagementThread implements Runnable, OpenVPNManagement {
	...
	private OpenConnectService mOpenVPNService;
    ...
    public OpenConnectManagementThread(Context context, VpnProfile profile, OpenConnectService openVpnService) {
    	mContext = context;
		mProfile = profile;
...
//			}
		...
		public int onProcessAuthForm(LibOpenConnect.AuthForm authForm) {
			...
			Log.d(TAG, "onProcessAuthForm: mLastFormDigest: " + mLastFormDigest);
	...
	private boolean setPreferences() {
		...
		Log.d(TAG, "setPreferences: disable_xml_post: " + getBoolPref("disable_xml_post")
				+ "; require_pfs: " + getBoolPref("require_pfs"));

		String os = getStringPref("reported_os");
		Log.d(TAG, "setPreferences: reported_os: " + os);
		...
		Log.d(TAG, "setPreferences: dpd_override: " + getBoolPref("dpd_override"));
		if (getBoolPref("dpd_override")) {
			try {
				int dpd = Integer.parseInt(getStringPref("dpd_value"));
				Log.d(TAG, "setPreferences: dpd_value: " + dpd);
				if (dpd > 0) {
					mOC.setDPD(dpd);
				}
			} catch (Exception e) {
		...
		int ret = 0;
		Log.d(TAG, "setPreferences: software_token: " + s + "; token_string: " + token);
		...
		Log.d(TAG, "setPreferences: ret: " + ret);
		if (ret < 0) {
	...
	private void updateLogLevel() {
		Log.d(TAG, "updateLogLevel: trace_log: " + mAppPrefs.getBoolean("trace_log", false));
	...
	private void errorAlert(String message) {
		Log.e(TAG, "OpenConnectManagementThread: errorAlert: message: " + message);
		setState(STATE_DISCONNECTED);
...
//				message));
	...
	public void requestStats() {
		...
		synchronized (mMainloopLock) {
			Log.d(TAG, "requestStats: mOC: " + (mOC != null));
			if (mRequestPause || mRequestDisconnect || mOC == null) {
				noStats = true;
			} else {
				mOC.requestStats();
		...
		Log.d(TAG, "requestStats: noStats: " + noStats);
		if (noStats) {
