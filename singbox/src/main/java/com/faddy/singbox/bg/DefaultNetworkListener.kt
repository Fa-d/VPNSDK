...
object DefaultNetworkListener {
    ...
    private fun register() {
        when (Build.VERSION.SDK_INT) {
            in 31..Int.MAX_VALUE -> @TargetApi(31) {
                CustomApplication.connectivity.registerBestMatchingNetworkCallback(
                    request,
                    Callback,
                    mainHandler
                )
            ...
            in 28 until 31 -> @TargetApi(28) {  // we want REQUEST here instead of LISTEN
                CustomApplication.connectivity.requestNetwork(request, Callback, mainHandler)
            }

            in 26 until 28 -> @TargetApi(26) {
                CustomApplication.connectivity.registerDefaultNetworkCallback(Callback, mainHandler)
            }

            in 24 until 26 -> @TargetApi(24) {
                CustomApplication.connectivity.registerDefaultNetworkCallback(Callback)
            }

            else -> try {
                fallback = false
                CustomApplication.connectivity.requestNetwork(request, Callback)
    ...
    private fun unregister() {
        runCatching {
            CustomApplication.connectivity.unregisterNetworkCallback(Callback)
        }
    }
}